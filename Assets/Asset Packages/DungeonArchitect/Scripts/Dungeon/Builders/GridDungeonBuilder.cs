...
namespace DungeonArchitect
{
    ...
    public class GridDungeonBuilder : DungeonBuilder
    {
        ...
        void ApplyBaseOffset()
        {
            ...
            var dungeonGridPosF = MathUtils.Divide(dungeonPosition, config.GridCellSize);
        ...
        void ApplyNegationVolume(NegationVolume volume)
        {
            ...
            volume.GetGridTransform(out position, out scale);
        ...
        void AddPlatformVolume(PlatformVolume platform)
        {
            ...
            cell.Id = GetNextCellId();
            IntVector position, scale;
            platform.GetGridTransform(out position, out scale);
            position = position - scale / 2;
            var bounds = new Rectangle();
            bounds.Location = position;
            bounds.Size = scale;
        ...
        void BuildMesh_Room(Cell cell)
        {
            ...
            for (int dx = 0; dx < cell.Bounds.Size.x; dx++)
            {
                ...
                EmitMarker(DungeonConstants.ST_WALLHALF, transform, elevation, HalfWallOffset, gridPosition, cell.Id);
                ...
                EmitMarker(DungeonConstants.ST_WALLHALFSEPARATOR, transform, elevation, HalfWallOffset, gridPosition, cell.Id);
        ...
        void BuildMesh_Corridor(Cell cell)
        {
            ...
            for (int dx = 0; dx < cell.Bounds.Size.x; dx++)
            {
                ...
                if (drawFence || isElevatedFence)
                {
                    ...
                    if (isElevatedFence)
                    {
                        EmitMarker(DungeonConstants.ST_WALLHALF, transform, elevationHeight, HalfWallOffset, gridPosition, cell.Id);
                    }
                }
                if (drawFence || drawPillar)
                {
                    ...
                    if (isElevatedFence)
                    {
                        EmitMarker(DungeonConstants.ST_WALLHALFSEPARATOR, transform, elevationHeight, HalfWallOffset, gridPosition, cell.Id);
                    }
                }
